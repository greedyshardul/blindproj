...
export const trilaterate = function(beacons) {
  ...
  //normalization
  const latmax=math.max(beacons[0].lat,beacons[1].lat,beacons[2].lat)
  const lonmax=math.max(beacons[0].lon,beacons[1].lon,beacons[2].lon)
  const latmin=math.min(beacons[0].lat,beacons[1].lat,beacons[2].lat)
  const lonmin=math.min(beacons[0].lon,beacons[1].lon,beacons[2].lon)
  var nlat=lat
  var nlon=lon
  while(nlat>latmax)
  {
